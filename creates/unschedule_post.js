...
const perform = async (z, bundle) => {
  const response = await z.request({
    method: 'POST',
    url: `${API_BASE_URL}/v1/posts/${bundle.inputData.postId}/unschedule`,
  });
...
module.exports = {
  ...
  noun: 'Post',
  display: {
    label: 'Unschedule Post',
    description: 'Moves a scheduled post back to draft and clears its publish time.',
  },
  operation: {
    perform,
    inputFields: [
      {
        ...
        type: 'string',
        required: true,
        dynamic: 'list_scheduled_posts.id.name',
        search: 'find_post.id',
        helpText: 'ID of the scheduled post to move back to draft. Only scheduled posts can be unscheduled.',
      },
    ],
    sample: {
